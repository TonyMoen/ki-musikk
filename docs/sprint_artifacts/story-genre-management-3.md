# Story 3: Implement AI Prompt Assistant Modal

**Epic:** Genre Management & AI Assistant
**Story ID:** genre-management-3
...
**4. Quick Answers Component (src/components/ai-assistant/quick-answers.tsx):**
```typescript
'use client'
...
**Prerequisites:**
- Story 11-1: Genre Edit Mode (provides genre management foundation)
- UI Modernization Epic Story 3 (provides "+ Legg til sjanger" button)

**Blocks:**
- Story 11-4: Genre Library Modal (will store custom genres in "Aktive" tab)
...
**Why 5 Steps:**
- Covers all Suno prompt components (style, instruments, mood, production, extras)
- Not overwhelming (each step is simple)
- Optional steps reduce friction
- Industry best practice (progressive disclosure)

**Why Comma-Separated:**
- Suno's recommended prompt format
- Easy to parse and edit manually
- Clear structure: "style, instruments, mood, production, extras"

**Why No Backend Yet:**
- MVP focuses on session persistence
- Story 4 (Genre Library) will add localStorage
- Future: Supabase storage with user accounts

**Conversation Design Principles:**
- Start with most important (style/genre)
- Build complexity gradually
- Optional steps at end (production, extras)
- Natural language (Norwegian, friendly tone)
- Quick answers for speed, free-form for flexibility

**Future Enhancements:**
- AI-powered suggestion engine (analyze successful songs)
- Prompt preview (test before saving)
- Conversation history (review past creations)
- Template library (rock, pop, electronic presets)
...
**Next Steps:**
1. Create AI assistant modal component structure
2. Implement conversation flow state machine in hook
3. Build chat container with message rendering
4. Add quick answer buttons with click handlers
5. Implement prompt generation logic
6. Add genre naming and save functionality
7. Integrate with genre-selection.tsx
8. Style chat messages (AI vs User)
9. Test full flow end-to-end
10. Commit changes and mark ready for review

**After This Story:**
- Move to Story 4: Genre Library Modal (CR-005)
- Epic 11 complete!
...
**Related Files:**
- `src/components/ai-assistant/modal.tsx` - NEW (Main modal)
- `src/components/ai-assistant/chat-container.tsx` - NEW (Message list)
- `src/components/ai-assistant/message.tsx` - NEW (Individual message)
- `src/components/ai-assistant/quick-answers.tsx` - NEW (Quick buttons)
- `src/components/ai-assistant/input-container.tsx` - NEW (Input field)
- `src/hooks/use-ai-assistant.ts` - NEW (Conversation logic)
- `src/lib/constants.ts` - UPDATE (Flow config)
- `src/components/genre-selection.tsx` - UPDATE (Integration)

**Design References:**
- Conversational UI patterns (Intercom, Drift)
- Suno prompt format documentation
- Material Design: Dialogs and overlays
