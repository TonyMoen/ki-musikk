# Story 4: Add Genre Library Modal with Active/Archived/Standard Tabs

**Epic:** Genre Management & AI Assistant
**Story ID:** genre-management-4
...
**5. Integration with Genre Selection (src/components/genre-selection.tsx):**
```typescript
import { useState } from 'react'
...
export function GenreSelection() {
  ...
  return (
    <div>
      ...
      {/* Genre grid using library.activeGenres */}
...
- ~10MB quota (sufficient for 100+ genres)

**Why Three Tabs:**
- **Aktive:** Current working set (in main grid)
- **Arkiverte:** Safety net (restore removed genres)
- **Standard:** Discovery (explore pre-made templates)

**Why Standard Templates:**
- Reduce friction for new users
- Demonstrate Suno prompt format
- Provide inspiration
- Cover common use cases (rock, electronic, jazz, etc.)

**Data Flow:**
1. User removes genre in edit mode → moves to Arkiverte (localStorage)
2. User creates genre via AI assistant → adds to Aktive (localStorage)
3. User adds standard template → copies to Aktive (localStorage)
4. User restores archived genre → moves back to Aktive (localStorage)

**Future Enhancements:**
- Cloud sync via Supabase (requires authentication)
- Genre sharing (export/import JSON)
- Drag & drop reordering
- Genre analytics (most used, success rate)
- Batch operations (archive multiple at once)
...
**Status:** BACKLOG (Waiting for Stories 11-1, 11-2, 11-3)

**Next Steps:**
1. Create standard-genres.ts with 8 templates
2. Create useGenreLibrary hook with localStorage
3. Build genre library modal with tabs
4. Create genre card component
5. Implement search filtering
6. Integrate with genre-selection.tsx
7. Add library button to header
8. Test archive/restore/delete flows
9. Test localStorage persistence
10. Commit changes and mark ready for review

**After This Story:**
- Epic 11 (Genre Management & AI Assistant) COMPLETE! 🎉
- All 4 change requests implemented (CR-003, CR-004, CR-005, CR-006)
...
**Story Ready for Development:** After Stories 11-1, 11-2, 11-3 complete
